		...
		<table id="example2" class="table table-bordered table-hover">
					...
					<tbody>
						...
						while($list=mysql_fetch_array($sql2)){
							$sql_jml_perubahan=mysql_query("select max(perubahan),konfirmasi from request where tiket='".$list['tiket']."'");
  ...
  ?>
	...
	<div class="box box-primary">
	  <div class="box-header with-border">
		...
		?>
		<form id="entri" name="entri"  enctype="multipart/form-data" action="?<?php $request="request&tiket=".$var['tiket']; echo paramEncrypt('page='.$request.''); ?>" class="form-horizontal"  method="post" onsubmit="return submitForm()">
			<div class="box-body">
				...
				<div id="divDokumen">
								...
								<span class="input-group-btn">
									...
									<script>
										function uploadFile() {
											...
											var ket_dokumen = $("#ket_dokumen").val();
											var alasan_dokumen = $("#alasan_dokumen").val();
					...
					<div class="form-group">					
						<label class="col-sm-2 control-label">Keterangan perubahan</label>
						<div class="col-sm-10">
							<textarea class="form-control" rows="2" name="ket_dokumen" id="ket_dokumen" placeholder="Isikan keterangan perubahan..." ><?php if ($_SERVER['REQUEST_METHOD'] == 'POST') echo$_POST['alasan'] ?></textarea>					  
						</div>
					</div>
					<div class="form-group">					
						<label class="col-sm-2 control-label">Alasan perubahan</label>
						<div class="col-sm-10">
							<textarea class="form-control" rows="2" name="alasan_dokumen" id="alasan_dokumen" placeholder="Isikan alasan perubahan..." ><?php if ($_SERVER['REQUEST_METHOD'] == 'POST') echo$_POST['alasan'] ?></textarea>
						</div>
					</div>
			...
			<div class="box-footer">
				...
				<!--<button type="submit" class="btn btn-info pull-right"><span class="fa fa-save"> Simpan</span></button>-->
...
function SimpanEntri(){
    ...
    var nama	    	= $("#nama").val();
    var perubahan   	= $("#perubahan").val();
    var ket_fitur   	= $("#ket_fitur").val();
    var alasan_fitur   	= $("#alasan_fitur").val();
	var ket_dokumen   	= $("#ket_dokumen").val();
    var alasan_dokumen  = $("#alasan_dokumen").val();
	cekFitur=0;cekJenis=0;pesan="";
	var checkedValueFitur = parseInt($('#cekfitur:checked').val());
	var checkedValueDok = parseInt($('#cekdok:checked').val());
	if(isNaN(checkedValueFitur)){
		checkedValueFitur=0;
	}
	if(isNaN(checkedValueDok)){
		checkedValueDok=0;
	}
	if(checkedValueFitur+checkedValueDok==0){	
		pesan+="\nJenis Perubahan harus ada yang terplih";	
		alert(pesan);
	}
	if(checkedValueFitur+checkedValueDok==1){
		for(var i=0; i < document.getElementsByName("fitur[]").length;i++){
			if(document.getElementsByName("fitur[]")[i].checked) cekFitur++;
		} 
		if(cekFitur==0)
			pesan+="\nFitur harus ada yang terplih";	
		if(tiket=="" ||  nama=="" || ket_fitur=="" || alasan_fitur==""){
			//alert("Semua isian harus terisi");
			document.getElementById("ket_fitur").closest(".form-group").classList.add('has-error');
			document.getElementById("alasan_fitur").closest(".form-group").classList.add('has-error');
			cek=false;
		}
		else cek=true;
	}
	if(checkedValueFitur+checkedValueDok==2){
		if(tiket=="" ||  nama=="" || ket_dokumen=="" || alasan_dokumen==""){
			//alert("Semua isian harus terisi");
			document.getElementById("ket_dokumen").closest(".form-group").classList.add('has-error');
			document.getElementById("alasan_dokumen").closest(".form-group").classList.add('has-error');
			cek=false;
		}
		else cek=true;
	}
	if(checkedValueFitur+checkedValueDok==3){
		for(var i=0; i < document.getElementsByName("fitur[]").length;i++){
			if(document.getElementsByName("fitur[]")[i].checked) cekFitur++;
		} 
		if(cekFitur==0)
			pesan+="\nFitur harus ada yang terplih";
		if(tiket=="" ||  nama=="" || ket_fitur=="" || alasan_fitur=="" || ket_dokumen=="" || alasan_dokumen==""){
			//alert("Semua isian harus terisi");
			document.getElementById("ket_fitur").closest(".form-group").classList.add('has-error');
			document.getElementById("alasan_fitur").closest(".form-group").classList.add('has-error');
			document.getElementById("ket_dokumen").closest(".form-group").classList.add('has-error');
	...
	if(cek==true){
	    $("#loadingImage").show();
          ...
          $.ajax({
            url: 'pages/ajax/request_add.php',
            type: 'POST',
            data: formData,
            async: true,
            cache: false,
            ...
            success: function (returndata) {
              ...
              if(pesan[0]=="Berhasil menambahkan perubahan permintaan"){
                  alert(pesan[0]);
